...
class CustomImageDataset(Dataset):
    def __init__(self, images, coordinates, countries, datasize, replace_country_index):
        ...
        self.country_index_path = os.path.join("models", f"datasize_{datasize}_country_to_index.json")
        ...
        if replace_country_index:
            if os.path.exists(self.country_index_path):
                ...
                print(f"File not found: {self.country_index_path}. Generating new mapping.")
                self._generate_and_save_country_index()
        else:
            self._generate_and_save_country_index()

    def _generate_and_save_country_index(self):
        unique_countries = sorted(set(self.countries))
        self.country_to_index = {country: idx for idx, country in enumerate(unique_countries)}
        os.makedirs(os.path.dirname(self.country_index_path), exist_ok=True)
